...
async function withTimeout<T>(promise: Promise<T>, ms: number, label = "op"): Promise<T> {
  ...
  const timeout = new Promise<never>((_, rej) => {
    ...
    t = setTimeout(() => rej(new Error(`${label} timeout after ${ms}ms`)) as any, ms) as unknown as number;
  });
  try {
...
async function fetchWithTimeout(input: Request | string, init: any = {}) {
    ...
    <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;font-size:15px;color:#1a1a1a;line-height:1.5;max-width:600px;margin:auto;">
      ...
      <div style="background:#fff;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 6px 6px;padding:20px;">
        <table width="100%" cellpadding="6" cellspacing="0" style="border-collapse:collapse;">
          <tr><td style="font-weight:600;width:120px;">Name:</td><td>${escapeHtml(lead?.name)}</td></tr>
...
// Return structured provider result (don’t always throw → better status codes)
async function sendWithResend(to: string[], subject: string, html: string) {
  if (!RESEND_API_KEY || !FROM_EMAIL) {
    return { ok: false, status: 500, details: "Missing RESEND_API_KEY or RESEND_EMAIL" } as const;
  }
  ...
  if (!resp.ok) {
    ...
    return { ok: false, status: resp.status, details: text } as const;
  }
  log("sendWithResend ok");
  return { ok: true, status: 200, data: JSON.parse(text) } as const;
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    // Log presence (no secrets)
    log("env present", {
      hasUrl: !!SUPABASE_URL,
      hasService: !!SERVICE_ROLE_KEY,
      hasResendKey: !!RESEND_API_KEY,
      hasFrom: !!FROM_EMAIL,
      resendKeyLen: RESEND_API_KEY.length,
    });

    // Env check AFTER OPTIONS
    ...
    // Create Supabase client only after env validation
    ...
    if (!body?.lead || typeof body.lead !== "object") {
      return new Response(JSON.stringify({ ok: false, error: "Missing or invalid 'lead' in body" }), {
        status: 400,
        headers: corsHeaders,
      });
    }

    // --- Normalize lead and basic validation ---------------------------------
    const lead = body.lead as any;
    const record = {
      name: String(lead?.name ?? "").trim(),
      email: String(lead?.email ?? "").trim().toLowerCase(),
      phone: String(lead?.phone ?? "").trim(),
      message: (String(lead?.message ?? "").trim() || null) as string | null,
    ...
    const { error: insertError } = await withTimeout(
      sb.from("leads").insert(record),
      4000,
      "insertLead",
    ) as any;
    if (insertError) {
      log("insertLead error", (insertError as any)?.message || String(insertError));
      return new Response(JSON.stringify({ ok: false, error: "Lead insert failed" }), {
        status: 500,
        headers: corsHeaders,
      });
    }
    ...
    if (error) {
      log("getRecipients error", (error as any)?.message || String(error));
      return new Response(JSON.stringify({ ok: false, error: "Recipient query failed" }), {
        status: 500,
        headers: corsHeaders,
      });
    }
    const to = Array.from(new Set(
      ((data ?? []) as Array<{ email_to?: string }>)
        .map((r) => (r?.email_to ?? "").trim())
        .filter(Boolean),
    ));
    log("getRecipients ok", to.length);

    if (!to.length) {
      return new Response(JSON.stringify({ ok: true, info: "No recipients opted-in" }), {
        status: 200,
        headers: corsHeaders,
      });
    }

    // --- Email send (10s cap total) -----------------------------------------
    const subject = `New lead: ${record.name || record.email || record.phone || "Website"}`;
    const html = renderLeadHTML(record);
    const sendResult = await withTimeout(sendWithResend(to, subject, html), 10000, "sendWithResend") as any;

    if (!sendResult.ok) {
      // Mirror provider’s status (e.g., 401) so you see the real cause
      const status = sendResult.status >= 400 && sendResult.status < 600 ? sendResult.status : 502;
      return new Response(JSON.stringify({ ok: false, provider: "resend", status, details: sendResult.details }), {
        status,
        headers: corsHeaders,
      });
    }
    ...
    // --- Fire-and-forget logging; NO .catch chaining (compat safe) ----------
    (async () => {
      try {
        await sb.from("email_logs").insert(to.map((sent_to) => ({ sent_to, subject })));
      } catch (e) {
        log("email_logs insert error", (e as any)?.message || String(e));
      }
    })();
    // --- end logging ---
    ...
    log("handler error", (err as any)?.message || String(err));
